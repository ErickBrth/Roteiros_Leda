...
public class QuickSelect<T extends Comparable<T>> {
	...
	public T quickSelect(T[] array, int k) {
		...
		if (array.length > 0 && k <= array.length && k > 0) {
			quickSelect = quickSelectKOrderStatistic(array, k, 0, array.length - 1);
		}
		return quickSelect;
	}

	private T quickSelectKOrderStatistic(T[] array, int k, int leftIndex, int rightIndex) {
		if (leftIndex >= 0 && rightIndex < array.length && leftIndex < rightIndex) {

			int pivotIndex = partition(array, leftIndex, rightIndex);

			if (k < pivotIndex + 1) {
				return quickSelectKOrderStatistic(array, k, leftIndex, pivotIndex);
			} else if (k > pivotIndex + 1) {
				return quickSelectKOrderStatistic(array, k, pivotIndex + 1, rightIndex);
			}
	...
	private int partition(T[] array, int leftIndex, int rightIndex) {
		...
		medianOfThree(array, leftIndex, middle, rightIndex);
		T pivot = array[middle];
		util.Util.swap(array, middle, rightIndex - 1);
		int pivotIndex = rightIndex - 1;

		for (int i = pivotIndex - 1; i > leftIndex; i--) {
			if (pivot.compareTo(array[i]) < 0) {
				pivotIndex--;
				util.Util.swap(array, pivotIndex, i);
			}

		}
		util.Util.swap(array, rightIndex - 1, pivotIndex);
		return pivotIndex;
	}

	private void medianOfThree(T[] array, int leftIndex, int middle, int rightIndex) {
